    ...
    <div ref={popupRef} className="absolute right-0 top-12 z-50 bg-white border border-gray-200 rounded-2xl shadow-2xl w-64 p-5">
      ...
      <div className="space-y-3">
        <div>
          ...
          <input type="number" min="0" max="100" value={filters.gradeMin}
        ...
        <div>
          ...
          <input type="number" min="0" value={filters.inspeksiMin}
            onChange={e => onChange('inspeksiMin', e.target.value)}
            className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-orange-400 focus:ring-1 focus:ring-orange-400" />
        </div>
      </div>
...
export default function AdminUsersPage() {
  ...
  useEffect(() => {
    setLoading(true)
    setError(null)
    AdminService.getUsers({ search, page, limit: LIMIT })
      .then(data => {
        setUsers(data.users || [])
  ...
  function resetFilter()      {
    const d = { gradeMin: '', inspeksiMin: '' }
    setFilters(d); setApplied(d)
  }
  ...
  const totalInspeksi = filtered.reduce((s, u) => s + u.totalInspeksi, 0)
  const avgGrade = filtered.length > 0
    ? Math.round(filtered.reduce((s, u) => s + u.gradeAPercent, 0) / filtered.length)
    : 0
    ...
    <div className="flex-1 bg-[#f8f9fa] min-h-screen p-8">
      <div className="max-w-5xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-8">
          ...
          <span className="text-sm text-gray-500">{total} pengguna terdaftar</span>
        </div>
        ...
        <div className="bg-white border border-gray-200 rounded-2xl shadow-sm overflow-hidden">
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {loading ? (
                <tr><td colSpan={8} className="text-center py-10 text-gray-300 text-sm">Memuat...</td></tr>
              ) : error ? (
                <tr><td colSpan={8} className="text-center py-10 text-red-400 text-sm">{error}</td></tr>
              ) : filtered.length === 0 ? (
                ...
                return (
                  <tr key={u.id} className="border-t border-gray-100 hover:bg-gray-50 transition-colors">
                    <td className="px-6 py-3 text-sm">
                      {rank <= 3 ? ['🥇', '🥈', '🥉'][rank - 1] : <span className="text-gray-400 text-xs">{rank}</span>}
                    </td>
          ...
          {totalPages > 1 && (
            <div className="flex items-center justify-between px-5 py-4 border-t border-gray-100">
              <p className="text-xs text-gray-400">Halaman {page} dari {totalPages} · {total} pengguna</p>
