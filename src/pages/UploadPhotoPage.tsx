...
import api from '../lib/api'; // Sesuaikan path ini jika merah
...
export const UploadPhotoPage = ({ onBack }: UploadPageProps) => {
  ...
  const handleSaveToBatch = async () => {
    ...
    try {
      ...
      // Axios otomatis mengekstrak JSON, jadi kita tidak perlu response.json()
      // Jika berhasil, kita bisa langsung tampilkan alert
      ...
      if (eyeCameraRef.current) eyeCameraRef.current.value = '';
      if (gillCameraRef.current) gillCameraRef.current.value = '';
  ...
  return (
    <div className="flex-1 bg-white min-h-screen p-8 max-w-6xl mx-auto flex flex-col relative">
      ...
      <div className="flex justify-between items-center mt-8 pt-6 border-t border-gray-100">
        ...
        >
          {loading ? 'Menyimpan...' : 'Save to batch'}
        </button>
      </div>
    </div>
  );
